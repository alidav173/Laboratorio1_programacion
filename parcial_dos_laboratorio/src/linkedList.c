...
/** \brief Permite obtener un elemento de la linkedlist
 *
 * \param this LinkedList* Puntero a la lista
    ...
    if(this != NULL && index >= 0 && index < ll_len(this)){
    	...
    	if(nodoBuscado != NULL){
    		if(index == 0){ //si es el primer elemento
    			...
    			this->pFirstNode = nodoBuscado->pNextNode;
    		}
    		else{
...
/**
 * @brief  Copia una lista los elementos  cuales
 * pasen la funcion pasada por parametro
 * \param  LinkedList *this lista a filtrar
 * \param  pFunc Funcion que funciona de filtro
 * \return Retorna la lista o NULL.
 */
LinkedList* ll_filter(LinkedList *this, int (*pFunc)(void*)) {

	/** llama a pFunc y le pasa un elemento, si pFunc devuelve 1, lo carga*/

	int len; //guardo el len de la linkedlist
	void *pElement;
	LinkedList *newLinkedList = NULL; //LL para los elementos de la funcion filtro

	if (this != NULL && *pFunc != NULL) {

		newLinkedList = ll_newLinkedList();
		if (newLinkedList != NULL) {
			len = ll_len(this); //guardo el tamanio de la lista original

			for (int i = 0; i < len; i++) {
				pElement = ll_get(this, i); //recorro la lista original.
				if (pFunc(pElement) == 1) { // si la funcion filtro devuelve 1, agregado
					ll_add(newLinkedList, pElement); //agrego los elementos a la nueva linkedlist
				}
			}
		}
	}
 ...
 * @param LinkedList* this lista a ser parseado
...
LinkedList* ll_map(LinkedList* this, void* (*pFunc)(void*)){
	...
	if(this!=NULL && *pFunc!=NULL){
		...
		for (int i = 0; i < len; i++) {
			...
			if(pElement!=NULL){
				ll_add(mappedList, pElement); //me los copio a la lista mapeada.
			}
		}
	}
